categoryCreateLog = async (ctx, record)=>{
    const create = {
        data : {
            category : {
                connect : {
...
const categoryDBCreate = async (ctx, data)=>{
    ...
    const result = await ctx.db.category.create(query)
...
const createService = async (ctx)=>{
    ...
    for(let item in ctx.data){
        try{
            ...
            let category = await categoryDBCreate(ctx, dataItem)
        ...
        const logged = await categoryCreateLog(ctx, created[0])
